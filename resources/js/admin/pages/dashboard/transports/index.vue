    ...
    <transition name="fade" mode="out-in" appear>
      ...
      <div v-else>
        <HeaderFilterInfo ref="filter"
                          ...
                          :title="$t('transports.index.filter_title')"
                          :filter="filter"
                          @create="busCreateTransport.$emit('openModal')"
                          @get="get"
                          @setViewLength="setViewLength"
        />

        <div class="row gy-3 mt-3">
          <div v-for="item in transports.data" :key="item.id" class="col-12">
            <ItemCardOneField :item="item"
                              :fields="inputs"
                              :update-modal-title="'transports.edit-modal.title'"
                              @update="update"
                              @destroy="deleteItem"
            />
          </div>
        ...
        <EditItemsModal :bus="busCreateTransport"
                        :title="'transports.create-modal.title'"
                        :fields="inputs"
                        :inputs="{}"
        />
      </div>
...
export default {
  ...
  data: () => ({
    title: i18n.t('transports.index.title'),
    viewLength: 10,
    transports: {},
    filter: new Vue(),
    busCreateTransport: new Vue(),
    inputs: [
      'id', 'name'
    ]
  }),
  ...
  async mounted () {
    await this.busCreateTransport.$on('save', this.store)
  ...
  methods: {
    ...
    get (settings) {
      axios.get('/api/transports', {
         ...
         * @var {array} r.data.payload.transports
         */
        .then(r => {
          this.transports = r.data.payload.transports
          this.filter.$emit('updateData', r.data.payload.transports)
     ...
     */
    deleteItem (id) {
      axios.delete('/api/transports/' + id)
        /**
         * @var {object} r
         * @var {object} r.data
         * @var {object} r.data.payload
         * @var {object} r.data.payload.transports
         */
        .then(r => {
          this.$vs.notification({
            ...
            position: 'top-right',
            color: 'success',
            title: this.$t('notification.delete.success.title', {name: r.data.payload.transport.name}),
            text: this.$t('notification.delete.success.text', {name: r.data.payload.transport.name})
          })
          this.get({
    ...
    update (params) {
      ...
      axios.put('/api/transports/' + params.id, {
        name: params.name
      })
        .then(r => {
          if(r.data.success) {
